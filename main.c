...
int main(int argc, char* argv[]) {
    ...
    // h_col = 10; w_col = 7; // debug

    Page* curr_page = new_page(cols, h_col, w_col, spacing, NULL);
    ...
    for (int i = 0; input_content[i] != '\0'; i += w_col) {
        ...
        // printf(" %d\n", strlen(line_part_content));

        if (!h_col_reached) {
            curr_page->curr_line = append_line(curr_page->curr_line, NULL);

            if (curr_page->lines_head == NULL) {
                curr_page->lines_head = curr_page->curr_line;
            }
        }
        ...
        if (h_col_reached) {
            curr_page->curr_line = curr_line->next_line;
        }
        ...
        if (line_counter == h_col) {
            ...
            curr_page->curr_line = curr_page->lines_head;
        }
    }
