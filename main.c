...
bool read_chunk(char* input_content, char* line_chunk_content, int w_col, int* base_idx) {
    ...
    for (int j = i; j < finish; j++) {
        ...
        if (curr_char != '\0') {
            if (!is_char(curr_char) && !is_text_started) { // TODO: se sono due mesa che sta cosa non va bene
                finish++;
                *base_idx += 1;
            } else {
...
void justify_string(char* string, int len) {
    ...
    int ratio = ceil_division(spaces_end, space_chunks);
